...
function stringTimes(string, n){
  var nString = string;
  for(var m = 0; m < n-1; m++){ nString += string};
  return nString;
}
...
function caught_speeding(spd, bday){
  if(bday){
    spd -= 5;
  }

  if (spd <= 60) { return 0; }
  else if(61 <= spd && spd <= 80){ return 1; }
  else if( 81 <= spd){ return 2; }
}
...
function makeBricks(sm, bg, goal){
  var m = Math.floor(goal/5);
  if(goal < 5){
    return sm >= goal;
  } else if (m <= bg){
      var v = goal - m*5;
...
// return goal%5 >= 0 && goal%5 - sm <= 0 && small + 5*bg >= goal;
